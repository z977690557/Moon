...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
       // self.backgroundColor=[UIColor whiteColor];
        ...
        _maxValue=1;
        self.backgroundColor =RGB(255, 95, 62);
    }
    return self;
}
//最大值
...
-(void)drawText:(NSString *)text withPoint:(CGPoint)point withType:(TitleType) type withIndex:(NSInteger)index
{
    ...
    else if (type==TitleForX)
    {
        ...
        {
            label.textColor=[UIColor whiteColor];
            //[UIColor colorWithRed:0.298f green:0.298f blue:0.298f alpha:1.00f];
        }
...
-(void)drawBrokenLine
{
    ...
    for (int i=0; i<self.count; i++) {
        ...
        //画折点
        [self drawPointWithCenterPoint:point radius:4 strokeColor:[UIColor whiteColor] fillColor:self.backgroundColor];
        
        //画X轴刻度点
        UIColor *pointColor=[UIColor whiteColor];
        //[UIColor colorWithRed:0.902f green:0.902f blue:0.902f alpha:1.00f];
        ...
        //画文字
        if (value!=0) {
        valueString=[NSString stringWithFormat:@"%.1f",(float)value];
        }else{
        valueString=[NSString stringWithFormat:@"%ld",(long)value];
        }
        ...
        if (_dataSource&&[_dataSource respondsToSelector:@selector(chart:titleForXLabelAtIndex:)]) {
            NSString *xstring=[_dataSource chart:self titleForXLabelAtIndex:[[_arr objectAtIndex:i] integerValue]-1];
